...
void Raft::initFromConfig(const std::string &filename) {
    ...
    if (ids != configMap.end()) {
        ...
        spdlog::info("find HEART_BEAT_INTERVAL in config= [{}]",
                     m_heatBeatInterVal);
    }
    ids = configMap.find("ELECTION_TIMEOUT");
    if (ids != configMap.end()) {
        m_leaderEelectionTimeOut_ = std::stoi(ids->second);
        spdlog::info("find ELECTION_TIMEOUT in config= [{}]",
                     m_leaderEelectionTimeOut_);
    }
...
void Raft::setRpcTimeOut(uint millisecond) { m_rpcTimeOut_ = millisecond; }
...
bool Raft::saveSnapShot(int index) {
    ...
    spdlog::info(
        "[{}]:{} saveSnapShot success,index = [{}],snapshotIndex = [{}]", m_me_,
        m_clusterAddress_[m_me_], index, snapshotIndex);
    return true;
}
...
int Raft::getLastLogTerm() const { return m_logs_[m_logs_.size() - 1].term(); }
...
int Raft::getStoreIndexByLogIndex(int logIndex) {
    ...
    if (storeIndex < 0) {
        spdlog::error(
            "getStoreIndexByLogIndex error,logIndex = [{}],m_snapShotIndex = "
            "[{}]",
            logIndex, m_snapShotIndex);
        return -1;
    }
    return storeIndex;
}
        ...
        for (int j = 0; j < m_peers_->numPeers(); j++) {
            if (m_matchIndex_[j] >= i) {
                ...
                if (count > m_peers_->numPeers() / 2) {
                    m_commitIndex_ = i;
                    hasCommit = true;
                    spdlog::info("[{}]:{},success commit log index = [{}]",
                                 m_me_, m_clusterAddress_[m_me_], i);
                    break;
                }
            }
        }
...
std::tuple<int, int, std::vector<LogEntry>> Raft::getAppendLogs(int peerId) {
    ...
    std::copy(this->m_logs_.begin() + (nextIndex - m_snapShotIndex),
              this->m_logs_.end(), logEntries.begin());
...
void writePersist(std::string logtermFile, std::string logcommandFile,
                  const std::vector<LogEntry> &logs, bool is_truncate) {
    if (is_truncate) {
        // 清空文件
...
static std::vector<std::string> all_persist_files = {
    "commitIndex.data",       "currentTerm.data",      "lastlogindex.data",
    "lastSnapshotIndex.data", "lastSnapshotTerm.data", "logentry.command.data",
    "logentry.term.data",     "votefor.data"};
...
void Raft::persist() {
    ...
    go[this] {
        std::filesystem::path dir =
            std::filesystem::path(m_persister_->absPersistPath_) / "persist";
        ...
        if (m_logs_.size() - 1 > last_to_disk_index &&
            ...
            logs.assign(m_logs_.begin() + last_to_disk_index + 1,
                        m_logs_.end());
        ...
        last_to_disk_term =
            m_logs_.back().term() > 1 ? m_logs_.back().term() : 1;
        co_mtx_.unlock();
        ...
        writePersist(dir / "logentry.term.data", dir / "logentry.command.data",
                     logs, is_truncate);
    };
}
