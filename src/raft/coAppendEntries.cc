...
void handleAppendSuccess(Raft *rf, int serverId,
                         const std::shared_ptr<AppendEntriesArgs> &args,
                         const std::shared_ptr<AppendEntriesReply> &reply);

void handleAppendFaild(Raft *rf, int serverId,
                       const std::shared_ptr<AppendEntriesArgs> &args,
                       const std::shared_ptr<AppendEntriesReply> &reply);
                    ...
                    go[this, i] {
                        ...
                        if (reply->term() > m_current_term_) {
                            ...
                            m_electionTimer->reset(
                                getElectionTimeOut(m_leaderEelectionTimeOut_));
                            co_mtx_.unlock();
                            persist();
                            return;
                        }
...
bool sendToAppendEntries(Raft *rf, int serverId,
    ...
    if (ok.ok()) {
        ...
        spdlog::error("disconnect to FOLLOWER [{}]:{}", serverId,
...
void handleAppendSuccess(Raft *rf, int serverId,
    ...
    if (args->entries_size() > 0 &&
        args->entries(args->entries_size() - 1).term() == rf->m_current_term_) {
        rf->tryCommitLog();
    }
    rf->persist();
}
...
void handleAppendFaild(Raft *rf, int serverId,
    ...
    if (reply->nextlogindex() != 0) {
        if (reply->nextlogindex() > rf->m_snapShotIndex) {
            rf->m_nextIndex_[serverId] = reply->nextlogindex();
            rf->m_appendEntriesTimer->reset(
                0);  // right now send appendEntries again
        } else {
...
void sendInstallSnapshotToPeer(Raft *rf, int serverId) {
    ...
    spdlog::info("send install snapshot to id[{}]:{}", serverId,
                 rf->m_clusterAddress_[serverId]);
    ...
    if (ok.ok() && reply.iscansendsnapfile()) {
        go[rf, serverId] {
            ...
            if (!toTransferSnapShotFiles(rf, serverId)) {
                ...
                spdlog::info("success transfer snapshot file to id[{}]:{}",
...
bool toTransferSnapShotFiles(Raft *rf, int serverId) {
    static std::vector<std::unique_ptr<RaftRPC::Stub>> &stubs =
        rf->m_peers_->getPeerStubs();
    spdlog::info("id[{}]:{} transfer snapshot file to id[{}]:{}", rf->m_me_,
                 rf->m_clusterAddress_[rf->m_me_], serverId,
                 rf->m_clusterAddress_[serverId]);
    if (serverId < 0 || serverId > rf->m_clusterAddress_.size() ||
        serverId == rf->m_me_) {
        spdlog::error("serverId:{} invalid in toTransferSnapShotFiles!",
                      serverId);
        return false;
    }
    std::filesystem::path snapshotFilePath =
        std::filesystem::path(rf->m_persister_->absPersistPath_) /
        rf->m_persister_->snapshotFileName_;
    ...
    if (!infile.is_open()) {
        spdlog::error("open snapshotFile file:{} faild",
    ...
    context.set_deadline(deadline);
    std::unique_ptr<::grpc::ClientWriter<::TransferSnapShotFileArgs>> writer =
        stubs[serverId]->TransferSnapShotFile(&context, &reply);
    while (!infile.eof()) {
