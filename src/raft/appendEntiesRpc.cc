        ...
        do {
            if (request->term() > m_rf_->m_current_term_) {
                ...
                if(response->success()){
                    ...
                    m_rf_->persist();
                    spdlog::info("success appendEntries,request[{}]",request->DebugString());
                    spdlog::info("success appendEntries,reply[{}]",response->DebugString());
                }else{
                    spdlog::info("falid appendEntries,request[{}]",request->DebugString());
                    spdlog::info("falid appendEntries,reply[{}]",response->DebugString());
                }
                for(int i = 0;i < m_rf_->m_logs_.size();i++){
                    spdlog::info("log:[{}],[{}/{}]",m_rf_->m_logs_[i].DebugString(),i,m_rf_->m_logs_.size());
                }
                printf("---");
