        ...
        do {
            if (request->term() > m_rf_->m_current_term_) {
                ...
                spdlog::debug("id[{}]:[{}] [{}] receive HeartBeat from id[{}]:{}", m_rf_->m_me_,
                              peersAddr[m_rf_->m_me_], m_rf_->stringState(m_rf_->m_state_), request->leaderid(),
                              peersAddr[request->leaderid()]);
                //do main logic
                ...
                if (response->success()) {
                    ...
                    if (m_rf_->m_commitIndex_ < request->leadercommit()) {
                        ...
                        *m_rf_->m_notifyApplyCh_ << (void *) 1;
                        spdlog::info("id[{}]:{}[{}] commit log[{}]", m_rf_->m_me_, peersAddr[m_rf_->m_me_],
